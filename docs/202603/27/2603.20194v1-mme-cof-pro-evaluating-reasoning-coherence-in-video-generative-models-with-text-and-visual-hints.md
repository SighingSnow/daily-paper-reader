...
title_zh: MME-CoF-Pro：基于文本和视觉提示评估视频生成模型的推理连贯性
...
tags: ["query:wmodel"]
score: 7.0
evidence: 视频生成模型作为世界模拟器，提供基准数据集
tldr: 针对视频生成模型缺乏跨帧因果一致性评估的问题，本文提出MME-CoF-Pro基准，包含303个样本及三种提示设置，旨在量化推理连贯性。实验表明现有模型推理连贯性较弱且与生成质量无关，文本提示易引发幻觉，视觉提示有助于结构化感知但在细粒度任务上表现不佳。
selection_source: fresh_fetch
motivation: 为了填补视频生成模型在跨帧因果一致性（即推理连贯性）评估方面的空白。
method: 提出MME-CoF-Pro基准，涵盖多类别样本，引入推理得分指标，并通过无提示、文本提示和视觉提示三种设置进行受控评估。
result: 评估发现视频模型推理连贯性较弱且与生成质量解耦，文本提示虽提升正确率但易引发幻觉，视觉提示在细粒度感知上存在困难。
conclusion: 现有视频生成模型的推理连贯性普遍不足，且文本与视觉提示对其推理过程的引导机制存在显著差异。
---

## 摘要
视频生成模型展现出涌现的推理行为。为了实现可靠部署，确保生成的事件在帧间保持因果一致性至关重要，我们将这一属性定义为推理连贯性。针对现有文献中缺乏推理连贯性评估的空白，我们提出了 MME-CoF-Pro，这是一个用于评估视频模型推理连贯性的综合视频推理基准。具体而言，MME-CoF-Pro 包含 303 个样本，涵盖 16 个类别，范围从视觉逻辑推理延伸至科学推理。该基准引入了 Reasoning Score 作为评估指标，用于评估过程级必要的中间推理步骤，并包含三种评估设置： 无提示、 文本提示和 视觉提示，从而能够对推理提示引导的潜在机制进行受控研究。对 7 个开源和闭源视频模型的评估结果揭示了以下见解：(1) 视频生成模型表现出较弱的推理连贯性，且与生成质量解耦；(2) 文本提示提升了表面正确性，但往往导致不一致和幻觉推理；(3) 视觉提示有益于结构化感知任务，但在细粒度感知上表现欠佳。网站：https://video-reasoning-coherence.github.io/
